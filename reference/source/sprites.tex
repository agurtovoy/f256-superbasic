...
Sprite data is built from PNG images up to 32x32. There are some examples in the \path{games/solarfox} directory in the SuperBASIC repository on GitHub.\footnote{\url{https://github.com/FoenixRetro/f256-superbasic/tree/main/games/solarfox}}\\

They can be created individually, or ripped from sprite sheets - this is what \path{ripgfx.py} is doing in the Makefile in \path{solarfox/graphics}; starting with the PNG file source .png it is informed where graphics are, and it tries to work out a bounding box for that graphic, and exports it to the various files.

\section{Building a sprite data set}

Sprite data sets are built using the \path{spritebuild.py} Python script (in the \path{utilities} subdirectory). Again there is an example of this in the \path{solarfox} directory.\\

Sprite set building is done using the \path{spritebuild.py} script which takes a file of sprite definitions. This is a simple text list of files, which can be either png files as is, or postfixed by a rotate angle (only 0,90,180 and 270) or v or h for vertical and horizontal mirroring.
